...
from sddo.sdmodel import modelBasic, modelJoint, sdZSJNLUEval_gate
...
def main(config):
    ...
    for iRep in range(int(config.get("experimentVer", "no_rep"))):
        ...
        slotGate = modelJoint.SlotGate(config)
        ...
        for iEpoch in range(noEpoch):
            ...
            for iBatch in range(noBatch):
                ...
                atten = seqProjector.atten
                transHseq = seqProjector.transHs

                #slotGate
                slotGate.forward(atten, Hx, transHseq)
                Ux = slotGate.outputU
            ...
            print(classification_report(batchSeqYid, batchSeqPred,
                                        digits=4, target_names=seqName))
            # evaluation
            rst = sdZSJNLUEval_gate.evalZSJ(sdData, batchSize, gpuFlag,
                                       utteranceEncoder, classYEncoder,
                                       seqYEncoder, classProjector, seqProjector, slotGate)
            perform.append(rst)
