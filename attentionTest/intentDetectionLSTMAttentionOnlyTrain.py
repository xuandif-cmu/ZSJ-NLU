...
if __name__ == "__main__":
    ...
    model = IntentDetectionOnlyAttentionModel(inputDim=300, hiddenDim=300, outputDim=clsLabSetSize, device=device)
    ...
    for epoch in range(epochNum):  # for each epoch
        ...
        # save model
        if epoch % 1 == 0:
            logger.saveModel(model, "IntentDetection_AttentionOnly_epoch%d"%(epoch))

    print ("[INFO] Training is done! Epoch = %d."%(epochNum))
    logger.saveModel(model, "IntentDetection_AttentionOnly_epoch_final_epoch%d"%(epoch))
