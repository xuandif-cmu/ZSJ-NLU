...
if __name__ == "__main__":
    ...
    model = SlotFillingModelLSTMAdvancedAttentionContextOnly(encoderInputDim=300, encoderHiddenDim=300, decoderOutputDim=300, outputDim=seqLabSetSize, device=device)
    ...
    logger = TrainingLogger(path=modelSavePath, evaluator=modelEvaluator, testBatchNum=2)
    ...
    for epoch in range(epochNum):  # for each epoch
        ...
        # evaluate and log
        logger.logSlotFillingModelStatusEpoch(model, epoch, epochStep=1, detailedReport=True)
        #logger.saveSlotFillingModelEpoch(model, epoch, epochStep=30)

        # save model
        if epoch % 1 == 0:
            logger.saveModel(model, "SlotFilling_LSTMAdvancedAttentionContextOnly_epoch%d"%(epoch))

    print ("[INFO] Training is done! Epoch = %d."%(epochNum))
    logger.saveModel(model, "FinalModel_SlotFilling_LSTMAdvancedAttentionContextOnly_final")
