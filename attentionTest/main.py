...
if __name__ == "__main__":
    ...
    model = SlotFillingModel(encoderInputDim=300, encoderHiddenDim=300, decoderOutputDim=300, outputDim=seqLabSetSize, device=device)

    # loss function
    criterion = SlotFillingLossCrossEntropy(datasetLoader=dataLoader, oneHotCoder=oneHotEncoder, device=device)
    criterion.setWeight(dataLoader, oneHotEncoder)

    # optimizer
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)

    # model evaluator
    modelEvaluator = Evaluator(dataLoader=dataLoader, criterion=criterion, wordEmbedder=wordEmbedder, oneHotCoder=oneHotEncoder, device=device)
    ...
    logger.saveModel(model, "FinalModel")
